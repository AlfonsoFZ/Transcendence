export class GameAI {
    ...
    update() {
        ...
        const paddleY = (_e = gameState.paddles[this.aiSide].y) !== null && _e !== void 0 ? _e : 0.5;
        let up = false, down = false;
        if (ballY < paddleY - this.errorFactor)
            up = true;
        else if (ballY > paddleY + this.errorFactor)
            down = true;
        (_f = this.game.getGameConnection().socket) === null || _f === void 0 ? void 0 : _f.send(JSON.stringify({
            type: 'PLAYER_INPUT',
            input: {
                player: this.aiSide,
                up,
